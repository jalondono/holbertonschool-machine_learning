...
def autoencoder(input_dims, hidden_layers, latent_dims):
    ...
    enc_hidden = K.layers.Dense(units=hidden_layers[0],
                                activation='relu')(enc_input)
    for i in range(1, len(hidden_layers)):
        enc_hidden = K.layers.Dense(hidden_layers[i],
                                    activation='relu')(enc_hidden)

    z_mean = K.layers.Dense(latent_dims)(enc_hidden)
    z_var = K.layers.Dense(latent_dims)(enc_hidden)
    z = K.layers.Lambda(sampling, output_shape=(latent_dims,))([z_mean, z_var])

    # decoder
    dec_input = K.Input(shape=(latent_dims,))
    dec_hidden = K.layers.Dense(hidden_layers[-1],
                                activation='relu')(dec_input)

    for i in range(len(hidden_layers) - 2, -1, -1):
        dec_hidden = K.layers.Dense(hidden_layers[i],
                                    activation='relu')(dec_hidden)

    dec_hidden = K.layers.Dense(input_dims,
                                activation='sigmoid')(dec_hidden)

    encoder = K.models.Model(inputs=enc_input, outputs=[z, z_mean, z_var])
    decoder = K.models.Model(inputs=dec_input, outputs=dec_hidden)

    encoderOut = encoder.layers[-1].output
    decoderOut = decoder(encoderOut)
    auto = K.models.Model(inputs=enc_input, outputs=decoderOut)
