...
class Yolo():
    ...
    def show_boxes(self, image, boxes, box_classes, box_scores, file_name):
        ...
        :param boxes: a numpy.ndarray containing the boundary
         boxes for the image
        :param box_classes: a numpy.ndarray containing the class
         indices for each box
        :param box_scores: a numpy.ndarray containing the box
         scores for each box
        :param file_name: the file path where the original
         image is stored Displays the image with all
          boundary boxes, class names, and box scores
        :return:
        """
        for idx, box in enumerate(boxes):
            ...
            pos_text = (int(x), int(y - 5))
            text = "{:.2f}".format(box_scores[idx])
            fontScale = 0.5
            # Red color in BGR
            color = (0, 0, 255)
